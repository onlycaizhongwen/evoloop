...
- Smoke: `scripts/run_demo_readiness_smoke.py` seeds an isolated `.tmp/demo-readiness-smoke` workspace, calls `/tasks/health.json`, prints each check, and exits non-zero unless the overall summary is `pass`.
...
- JSON endpoint verification: `python -m pytest -q tests/test_web_ui.py -k "health"` passed with `5 passed, 54 deselected`.
- Smoke verification: `python -m py_compile scripts/run_demo_readiness_smoke.py` passed; `python scripts/run_demo_readiness_smoke.py` passed with `demo_readiness_smoke=passed`; `python -m pytest -q tests/test_demo_readiness_smoke.py` passed with `1 passed`.
...
- Status: completed.
- Summary: `/tasks/audit` and `/tasks/audit.md` now support opt-in `scope=all` to include rotated `web-job-audit.*.jsonl` archives while preserving active-only default behavior. The page exposes a Scope selector, filtered Markdown exports preserve the selected scope, and archived events can be searched by event, job, run, message, request context, or detail fields.
- Artifacts: `orchestrator/infrastructure/persistence/web_job_audit_log.py`, `orchestrator/interfaces/web/main.py`, `orchestrator/interfaces/web/templates/task_audit.html`, `tests/test_web_job_audit_log.py`, `tests/test_web_ui.py`, `docs/codex/v1/plans/web-audit-archive-search.md`, `docs/codex/v1/trace/web-audit-archive-search-trace.md`.
- Verification: `python -m py_compile orchestrator/infrastructure/persistence/web_job_audit_log.py orchestrator/interfaces/web/main.py` passed; `python -m pytest -q tests/test_web_job_audit_log.py tests/test_web_ui.py -k "audit"` passed with `12 passed, 54 deselected`; `python -m pytest -q tests/test_web_ui.py tests/test_web_job_audit_log.py` passed with `66 passed`; `python -m pytest -q` passed with `154 passed`; `git diff --check` passed with only Windows CRLF warnings.
...
- Status: completed.
- Summary: `/tasks/audit` and `/tasks/audit.md` now expose read-time source provenance for audit records, distinguishing active `web-job-audit.jsonl` records from rotated archive records without rewriting existing JSONL files; the page/export also support `source=active|archive`, exact `source_file` filtering, and record counts in source/source-file selectors.
- Artifacts: `orchestrator/infrastructure/persistence/web_job_audit_log.py`, `orchestrator/interfaces/web/main.py`, `orchestrator/interfaces/web/templates/task_audit.html`, `tests/test_web_job_audit_log.py`, `tests/test_web_ui.py`, `docs/codex/v1/plans/web-audit-source-provenance.md`, `docs/codex/v1/trace/web-audit-source-provenance-trace.md`, `.codex/plans/main/web-audit-source-provenance/process.md`.
- Verification: `python -m py_compile orchestrator/infrastructure/persistence/web_job_audit_log.py orchestrator/interfaces/web/main.py` passed; `python -m pytest -q tests/test_web_job_audit_log.py tests/test_web_ui.py -k "audit"` passed with `12 passed, 55 deselected`; `python -m pytest -q tests/test_web_ui.py tests/test_web_job_audit_log.py` passed with `67 passed`; `python -m pytest -q` passed with `155 passed`; `git diff --check` passed with only Windows CRLF warnings.
