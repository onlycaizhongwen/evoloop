...
V4 在 V3 的基础上补充 4 个工程细节：

- Reviewer 输出非法 JSON 时增加最多 2 次重试兜底，避免偶发格式错误直接中断任务。
- Quality Gate 明确短路逻辑：Hard Check 失败时不调用 Reviewer，不浪费 Token。
- Diff 风险评分补充可量化扣分规则，便于 `gate.py` 稳定实现。
- Orchestrator 增加阶段日志与心跳日志，避免长任务期间被误判为卡死。
...
| Review Parser | 提取 Reviewer JSON，做 Schema 校验；非法 JSON 最多重试 2 次 | 是 |
...
stateDiagram-v2
    ...
    ReviewRetry --> Review: 要求 Reviewer 重新输出合法 JSON
...
| 修改源码但未新增或更新测试 | 扣 2 分 | Reviewer 必须检查测试缺口 |
...
```python
def calculate_diff_risk_score(diff_stats):
    score = 10
    ...
    if diff_stats.only_tests_changed:
        score -= 1
    if diff_stats.source_changed_without_tests:
        score -= 2
    return max(0, score)
```
...
2. 构造修复提示，要求 Reviewer 只返回符合 Schema 的 JSON，不附加解释。
...
def run_and_parse_review(task, max_retries=2):
    ...
    for retry_count in range(max_retries + 1):
        ...
        raw = reviewer.repair_json_output(
            ...
            instruction="只返回合法 JSON，不要输出 Markdown 或解释文字。"
        )
```
...
def run_task(task):
    ...
    while state.attempt < task.max_attempts:
        ...
        quality = gate.evaluate(task, hard_result, review, review_json_retry_count)
        state.save(quality)
...
| Reviewer 输出非法 JSON | 流程偶发中断 | malformed JSON 最多重试 2 次 |
...
- Reviewer JSON 非法时能最多重试 2 次并保存原始输出。
- 能生成 `quality_report.json`，包含 `review_json_retry_count` 与 `diff_risk_score`。
...
2. `task.json` 读取、状态初始化与路径安全校验。
3. Check Runner 与 Hard Check 短路逻辑。
4. Reviewer JSON Schema、解析与 malformed JSON 重试。
5. Diff 风险统计与 Quality Gate 评分。
6. Agent Adapter 接入 OMX/Codex CLI。
7. 状态持久化、最终报告与 pending-rules 候选规则生成。

## 17. 可行性与自信度

基于 V4 的收敛范围和补强细节，评估如下：
...
结论：V4 已具备进入执行计划阶段的条件。建议下一步直接产出 MVP 计划文档，优先实现 Hard Check 短路、Reviewer JSON 重试、Diff 风险评分和 heartbeat，这四点会显著提升第一版系统的稳定性。
