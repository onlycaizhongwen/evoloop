...
V7 在 V6 的基础上将两个细节固化为实现约束：

- Reviewer 输出非法 JSON 后，修复提示必须动态填入当前 `task_id`；解析成功后还必须校验 `review.task_id == task.task_id`。
- 当 `change_type=config` 时，Orchestrator 必须先按 `forbidden_paths` 判断是否阻断；未命中禁止路径时，再将默认权限映射为 `elevated` 并记录原因。
...
| `change_type` 为 `config` | 不按源码测试覆盖规则扣分 | 先检查 forbidden_paths，未命中时提升至 elevated 权限 |
...
6. 即使 JSON 解析成功，也必须校验 `review.task_id == task.task_id`；不一致时视为 malformed review，进入同一重试流程。
...
注意：task_id 必须使用当前任务 ID：{task_id}
...
`build_review_json_repair_prompt` 必须把当前 `task.task_id` 注入模板，禁止在修复提示中使用固定示例 ID，避免 Reviewer 将错误任务 ID 写回 `review.json`。

建议解析逻辑：

```python
def validate_review_identity(review, task):
    if review.task_id != task.task_id:
        raise MalformedReview(
            reason="task_id_mismatch",
            expected=task.task_id,
            actual=review.task_id
        )
```
...
当 `change_type=config` 时，Orchestrator 应先检查目标路径是否位于 `forbidden_paths`。如果命中禁止路径，必须直接阻断；如果未命中禁止路径，应自动将默认权限提升至 `elevated`，并在日志中注明原因，例如 `reason=change_type_config_requires_elevated`。

权限映射顺序必须固定为：

1. `forbidden_paths` 命中检查。
2. `change_type=config` 映射为 `elevated`。
3. 其他任务默认使用 `workspace-write`。
4. elevated 权限需要人工确认或显式配置允许。
...
| Reviewer 输出非法 JSON 或 task_id 不匹配 | 流程偶发中断或审查结果串任务 | malformed JSON 最多重试 2 次，重试提示携带字段模板和动态 task_id，并强校验 task_id |
...
| 配置类任务权限边界不清 | 配置改动被低权限误放行 | 先判断 forbidden_paths，再将 `change_type=config` 映射至 elevated |
...
- 当 `change_type=config` 时，能先检查 forbidden_paths，再自动切换至 elevated 权限并记录原因。
- 能调用 Coder/Fixer 完成一次代码修改循环。
- 能执行配置的测试、Lint、类型检查命令。
- Hard Check 失败时能短路 Reviewer，并进入 fix 或 halt。
- 能调用 Reviewer 并解析合法 `review.json`。
- Reviewer JSON 非法或 `task_id` 不匹配时能最多重试 2 次，重试提示包含字段模板和动态 `task_id`，并保存原始输出。
...
2. `task.json` 读取、`change_type` 校验、按 forbidden_paths 优先的权限映射、状态初始化与路径安全校验。
3. Check Runner 与 Hard Check 短路逻辑。
4. Reviewer JSON Schema、`task_id` 一致性校验、携带字段模板和动态 `task_id` 的 malformed JSON 重试。
...
结论：V7 已经具备进入执行计划阶段的条件。它比 V6 更适合直接指导实现，因为 Review 结果不仅动态生成当前任务 ID，还会强校验任务身份；配置类任务也明确了 forbidden 优先、elevated 后置的权限映射顺序。
