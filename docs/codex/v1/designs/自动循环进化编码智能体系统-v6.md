...
V6 在 V5 的基础上补充 2 个细节：

- Reviewer 输出非法 JSON 后，修复提示中直接提供字段模板，并动态填入当前 `task_id`，降低示例 ID 混淆概率。
- 当 `change_type=config` 时，Orchestrator 自动将默认权限提升至 `elevated` 并记录原因；若路径命中 `forbidden_paths`，仍直接阻断。
...
| `change_type` 为 `config` | 不按源码测试覆盖规则扣分 | 默认提升至 elevated 权限，并按路径策略判断风险 |
...
当 `change_type=config` 时，Orchestrator 应自动将默认权限提升至 `elevated`，并在日志中注明原因，例如 `reason=change_type_config_requires_elevated`。如果目标路径位于 `forbidden_paths`，则不允许提升权限，必须直接阻断。
...
| Reviewer 输出非法 JSON | 流程偶发中断 | malformed JSON 最多重试 2 次，重试提示携带字段模板和动态 task_id |
...
| 配置类任务权限边界不清 | 配置改动被低权限误放行 | `change_type=config` 自动提升至 elevated，命中 forbidden_paths 仍阻断 |
...
2. `task.json` 读取、`change_type` 校验、权限映射、状态初始化与路径安全校验。
3. Check Runner 与 Hard Check 短路逻辑。
4. Reviewer JSON Schema、解析与携带字段模板和动态 `task_id` 的 malformed JSON 重试。
...
结论：V6 已经具备进入执行计划阶段的条件。它比 V5 更适合直接指导实现，因为 Review JSON 修复提示绑定当前任务 ID，配置类任务的权限边界也更明确。
