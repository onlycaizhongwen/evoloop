...
Docker OMX Team Patch：修复 `calculator.py` 中的 `add` 函数错误。

演示流程：

1. 打开 Web UI 任务管理页。
2. 选择 `Docker OMX Team Patch` 推荐模板。
3. 提交任务并进入 Job 状态页。
4. 观察启动配置、执行链路和运行心跳。
5. 任务完成后进入 Run 详情页。
6. 展示执行摘要、阶段时间线、运行产物、Docker 执行证据和 Team Result。
7. 回到任务管理页展示已完成状态和重新运行入口。

### Slide 13: 真实验收证据

2026-06-02 已完成真实 Web UI + Docker 演示验收：
...
- 这不是纯 mock，而是通过 Web UI 提交真实 Docker sandbox 任务。
- Orchestrator 负责 patch apply、hard check、review 和 Quality Gate。
- 运行证据沉淀在 `docs/codex/v1/plans/demo-readiness.md`。
...
- 选择 1 到 2 个真实仓库试点。
- 沉淀更多真实演示样本和失败样本。
- 优化 Quality Gate 和 diff 风险评分。
- 增强 Web UI 的运行对比、批量筛选和审计导出。
- 扩展真实 Codex/OMX 多 Agent 编排与 Dashboard。
